...
class StartklarDropzoneFormField extends \ElementorPro\Modules\Forms\Fields\Field_Base
{
    ...
    public function __construct() {
        ...
        add_action( 'wp_footer', function() {  ?> <script>Dropzone.autoDiscover = false; </script> <?php   }, 1000);
        parent::__construct();
    }
    ...
    public function add_control_section_to_form( $element, $args ) {
        ...
        $element->start_controls_section(
            ...
            [
                'label' => __('DropZone', 'startklar-elmentor-forms-extwidgets' ),
	...
	public function render( $item, $item_index, $form ) {
        ...
        //if( !isset($user) || !is_object( $user ) || !is_a( $user, 'WP_User' ) ||  !$user->ID ) { return; }
...
EOT;
    ...
    public function sanitize_field( $value, $field ) {
        ...
        if (isset($options["dropzone_hash"]) && !empty($options["dropzone_hash"])){
        ...
        if (count($files)){
            foreach($files as $file){
                $save_path = "";
                $rel_path = "/elementor/forms/".$user_id."/".basename($file);
                ...
                $temp = 0;
                if (isset($options["path_type"]) && !empty($options["path_type"]) &&
                            in_array($options["path_type"],["url", "file_name_only","abs_path"])){
            ...
            $temp_folder = $uploads_dir_info['basedir']."/elementor/forms/".$user_id."/temp/";
